...
<head>
    ...
    <meta name="description" content="ПАВы и Адьюванты">
        ...
        <?php for($i = 0; $i < count($data['products']); $i++): ?>
            <div class="catalog-product">
                <div class="d-flex justify-content-between product-title">
                    ...
                    <a href="/herbicides/<?=$data['products'][$i]['id']?>"><button class="btn">Подробнее</button></a>
                </div>
                <div class="product-info">
                    <?php if($data['products'][$i]['substance'] != ''): ?>
                        <p><b>Действующее вещество: </b><?=$data['products'][$i]['substance']?></p>
                    <?php endif; ?>
                    <?php if($data['products'][$i]['appointment'] != ''): ?>
                        <p><b>Назначение: </b><?=$data['products'][$i]['appointment']?></p>
                    <?php endif; ?>
                    <?php if($data['products'][$i]['culture'] != ''): ?>
                        <p><b>Культура: </b><?=$data['products'][$i]['culture']?></p>
                    <?php endif; ?>
                    <?php if($data['products'][$i]['rate'] != ''): ?>
                        <p><b>Норма расхода: </b><?=$data['products'][$i]['rate']?></p>
                    <?php endif; ?>
